...
function restParametres(...args:(number|string|boolean)[]): string{
    const firstParameter:string = typeof args[0]
    const secondParameter:string =typeof args[1]
    if(firstParameter !== secondParameter){
        throw new Error("The two first parameter are not the same")
    }
    const validadeAll:boolean = args.every(a=>typeof a === firstParameter)
    if(!validadeAll){
